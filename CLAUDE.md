...
- **XPBD** — positional projection; fast; leaves a finite **equilibrium residual** (its settled state is a projection, not a force balance), so it settles the softest of the three solvers at this budget (tip ≈ 159 mm, ratio ≈ 3.7 vs FEM ≈ 43 mm — softer even than VBD). The canonical run that writes the shared mesh.
...
- **SemiImplicit** — explicit, force-based; the **differentiable** one → used by `diffsim.py` (the θ* effective-stiffness fit) and the Newton material test. Of the three it settles **closest** to FEM (tip ≈ 85 mm, ratio ≈ 1.97), with VBD (≈ 140 mm, ratio ≈ 3.2) between it and XPBD — so the softness ordering is SemiImplicit closest, VBD between, XPBD softest.
...
- Colab result numbers are **observations** (Python 3.12, dolfinx 0.11.0.post0, Warp 1.14.0), recorded with provenance — not eternal "verified" claims. The 2026-06-30 Colab run exercised every path; what remains open is numerical quality (tracked in STATUS Tier 3), not whether the API runs.
- The **contact scenarios (indentation/drop/friction) now wire all three solvers** via `--solver xpbd|vbd|semi_implicit` on the shared `soft_contact` path (as Newton's `example_rigid_soft_contact.py` does for every solver) — XPBD is the canonical default. VBD is the implicit counterpart to the implicit FEM *conceptually* (and for the hanging bar), **but for contact the holding solver is scenario-dependent**: on the 2026-06-30 Colab run (Warp 1.14.0) all three record results, yet for **indentation XPBD** holds the contact (pen 0 mm, U ≈ 13 J) while VBD/SemiImplicit on the soft_contact penalty path stay far too soft (the sphere sinks ≈ 34 / ≈ 33 mm), and in the **drop** only VBD makes a genuine two-way impact (XPBD never drives the free sphere down; the SemiImplicit drop blows up). So the VBD soft/rigid-contact path (AVBD, `rigid_body_particle_contact_buffer_size`) **does run on the pinned Newton** — the old "only XPBD records a result" caveat is resolved. Attribute "no calibrated contact force" to the **XPBD / fast soft-contact path**, not Newton generally. The **drop** is the hardest case (free sphere → two-way AVBD) and only a *partial* fairness fix vs. implicit Newmark — its transient also mixes material/contact-model/time-integration, so don't present it as a clean solver-only gap.
