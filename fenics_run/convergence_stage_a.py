"""(4) Convergence study -- FEM (FEniCSx) side, Stage A hanging block.
...
Two classic FEM convergence checks on the hanging block:
  ...
  1. MESH h-REFINEMENT: solve on structured meshes of increasing resolution
     ...
     how "tauglich" (adequate) the FEM model is, and how fast it converges (the
     discretisation error). cost = #DOFs and wall time.
...
Run (needs dolfinx):  python -m fenics_run.convergence_stage_a
"""
...
from fenics_run.run_stage_a import solve_hanging, evaluate_at_nodes
