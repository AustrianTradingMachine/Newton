"""Stage B -- contact prototype in FEniCSx (dolfinx), pure Python / UFL.
...
methods are provided, selected per variant in params.STAGEB_VARIANTS:
...
Both run the same indentation schedule as the Newton XPBD Stage B. Per step we
...
def _run(cell_name, penalty_factor, method, comm):
    ...
    nx, ny, nz = params.STAGEB_DIM
    h = params.STAGEB_CELL
    Lx, Ly, Lz = nx * h, ny * h, nz * h
    R = params.STAGEB_SPHERE_R
    cx, cy = Lx / 2.0, Ly / 2.0
    tol = 0.25 * h
    ...
    grav = (0.0, 0.0, -params.DENSITY * params.GRAVITY) if params.STAGEB_WITH_GRAVITY else (0.0, 0.0, 0.0)
    ...
    n_aug = params.STAGEB_AUG_ITERS if is_aug else 1
    n_steps = params.STAGEB_LOAD_STEPS
    label = _label(cell_name, penalty_factor, method)
    deltas, f_fem, e_strain, e_contact, penetration = [], [], [], [], []
    print(f"[stageB:{label}] slab {Lx:.2f}x{Ly:.2f}x{Lz:.2f} m, R={R} m, "
    ...
    for k in range(1, n_steps + 1):
        ...
        for a in range(n_aug):
            ...
            if max_penetration() < params.STAGEB_AUG_PEN_TOL:
                break
        ...
        print(f"[stageB:{label}] delta={delta * 1000:6.2f} mm  aug={a + 1}  F={abs(fz):.4g} N  "
...
def main():
    ...
    comm = MPI.COMM_WORLD
    print(f"[stageB] dolfinx {dolfinx.__version__}, variants={params.STAGEB_VARIANTS}")

    results = [_run(cell, factor, method, comm) for (cell, factor, method) in params.STAGEB_VARIANTS]
    ...
    np.savez(params.FEM_STAGEB_NPZ, **save)
    print(f"[stageB] wrote {params.FEM_STAGEB_NPZ}")
    for r in results:
        ratio = r["f_fem"][-1] / f_hertz[-1] if f_hertz[-1] > 0 else float("nan")
        print(f"[stageB] {r['label']}: F={r['f_fem'][-1]:.4g} N (Hertz ratio {ratio:.3f})  "
...
def _plots(results, f_hertz):
    ...
    plt.title("Stage B: contact force (element + penalty + AL)")
    plt.legend(); plt.grid(alpha=0.3)
    out = os.path.join(params.FIG_DIR, "stage_b_force.png")
    plt.tight_layout(); plt.savefig(out, dpi=130); print(f"[stageB] wrote {out}")
    ...
    plt.title("Stage B: residual penetration (AL ~ 0 at modest kn)")
    plt.legend(); plt.grid(alpha=0.3)
    out = os.path.join(params.FIG_DIR, "stage_b_penetration.png")
    plt.tight_layout(); plt.savefig(out, dpi=130); print(f"[stageB] wrote {out}")
    ...
    plt.title("Stage B: deformed top-surface profile (max indentation)")
    plt.legend(); plt.grid(alpha=0.3)
    out = os.path.join(params.FIG_DIR, "stage_b_profile.png")
    plt.tight_layout(); plt.savefig(out, dpi=130); print(f"[stageB] wrote {out}")
