...
> **Platform:** Newton needs a CUDA GPU → we run on **Kaggle** (free P100 16 GB or
> 2× T4 — a better GPU than Colab, and a headless **API**, `kaggle kernels
> push/output`). Google Colab (T4) still works as a fallback. FEniCSx is CPU and
> runs on the same instance (installed via fem-on-colab) or locally via
> conda-forge. See `00_setup_kaggle.ipynb` (primary) / `00_setup_colab.ipynb`.
...
00_setup_kaggle.ipynb       # Kaggle (primary): installation + full run (produces all data/*.npz)
00_setup_colab.ipynb        # Colab (fallback): installation + full run
kaggle/run_on_kaggle.py     # one-command launcher: upload code + kaggle kernels push + pull outputs
kaggle/kagglehub_sync.py    # kagglehub transport: code in / results out (no GitHub)
kaggle/kernel-metadata.json # Kaggle kernel spec template (regenerated by run_on_kaggle.py)
...
Kaggle gives a better free GPU (P100 16 GB or 2× T4) and a real **API**. The
whole thing — ship the code, launch the GPU run, pull the results — is one
command, **no GitHub needed**:

```bash
pip install kaggle kagglehub
kagglehub login                 # once -> writes the new OAuth token ~/.kaggle/access_token
python kaggle/run_on_kaggle.py --watch
```

`kaggle/run_on_kaggle.py` does all of it: (1) uploads the code as a Kaggle Dataset
via `kagglehub`, (2) stages the notebook + a generated `kernel-metadata.json`
(GPU + Internet on, source dataset attached) and runs **`kaggle kernels push`** —
the single step that actually starts the GPU run, (3) with `--watch`, polls
`kaggle kernels status` and pulls the outputs (`data/`, `figures/`) into `./out`.
Re-runs after the first can skip the code upload with `--no-src`.

**Auth** is the only thing you must provide, and Kaggle's **recommended** way is
the new OAuth login (`kagglehub login`, writes `~/.kaggle/access_token`) — the
modern `kaggle` CLI (≥ 2.2) and `kagglehub` both read it, so **no `kaggle.json` is
needed** (the legacy `kaggle.json` or `KAGGLE_USERNAME`+`KAGGLE_KEY` still work
too). The script never reads or stores a key; it lets the CLI / kagglehub
authenticate themselves and resolves your username via `kagglehub.whoami()`, so
`--prefix` is optional. Interactively you can instead open `00_setup_kaggle.ipynb`,
set **Accelerator → GPU** and **Internet → On**, and run the cells.

### Under the hood: `kagglehub` transport (used by the launcher)

`run_on_kaggle.py` delegates the **artifact transport** to `kagglehub` (Kaggle's
modern Python API for datasets / models / packages + auth). `kagglehub` is what
moves **code in** and **results out** without a GitHub remote; it does **not**
launch kernels — that is the `kaggle kernels push` step. You can drive the two
transport legs by hand with `kaggle/kagglehub_sync.py`:

```bash
python kaggle/kagglehub_sync.py upload-src   --prefix <user>          # ship the code
# ... kernel runs (launcher or UI) ...
python kaggle/kagglehub_sync.py download-out --prefix <user> -o ./out  # pull results
```

The `dataset_upload`/`dataset_download` signatures are the `verify-on-Kaggle`
spots in that helper. Plain `git clone` inside the notebook (Internet on) or a
manually attached Dataset in `dataset_sources` both still work as alternatives.

## On Google Colab (fallback)

Open `00_setup_colab.ipynb`, set the runtime to **T4 GPU**, run the cells in
order. It installs Newton, installs dolfinx via fem-on-colab, and runs the
complete pipeline including plots (the explicit Stage A solver, convergence and
friction are in the final §14 section).
...
## Still to verify on Kaggle / Colab

Nothing has executed on a GPU yet — all validation so far is local syntax +
synthetic-data contracts. Spots that are version dependent and marked in the code
with `TODO[verify-on-colab]` (read as *verify-on-Kaggle*):

1. **Newton:** the `particle_inv_mass` attribute used to clamp nodes (warp.sim convention),
   and the `--solver semi_implicit` explicit run reaching a settled state.
2. **FEniCSx (Stage A):** the `create_mesh` (tet, shared mesh) and `create_box` (hex) build calls.
3. **FEniCSx (Stage A):** the point-evaluation geometry API (`bb_tree` / `compute_colliding_cells`).
4. **FEniCSx (Stage B):** facet tagging (`locate_entities_boundary` / `meshtags`) and the
   `ufl.max_value` contact term — plus the penalty-contact Newton convergence (see tuning above).
5. **Newton (Stage B):** the kinematic-sphere drive (`_make_body_kinematic`, writing `body_q`
   into the state each step) and the particle pinning (`particle_inv_mass`).
6. **Drop (FEM):** Newmark update via `fem.Expression` interpolation, the no-Dirichlet dynamic
   tangent (mass-regularised), and impact stability (`dt` / penalty `kn` / damping fraction).
7. **Diffsim:** `finalize(requires_grad=True)`, the Tape/backward, and the SGD learning rate.
8. **Friction (FEM):** the component Dirichlet BCs (`V.sub(i).collapse()`) and the `min/max`
   Coulomb-law Newton convergence; **Newton:** `add_ground_plane()` + `soft_contact_mu`.
9. **Convergence:** the FEM `ndofs` query (`dofmap.index_map`) and longer GPU runtime for the
   XPBD iteration/substep sweep.
10. **Kaggle:** fem-on-colab selecting a build for Kaggle's Python, and the `kaggle kernels
    push` GPU/internet flags in `kaggle/kernel-metadata.json`.
