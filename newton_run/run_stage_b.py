"""Stage B -- Newton (XPBD) side: rigid-sphere indentation into a soft slab.

Adapted from Newton's `examples/multiphysics/example_rigid_soft_contact.py`, but
configured to match the FEniCSx Stage B benchmark (same slab geometry/material as
params.STAGEB_*) and driven QUASI-STATICALLY: the sphere is a *kinematic* body
...
Outputs -> data/newton_stage_b.npz:
...
Run on Colab (CUDA):  python -m newton_run.run_stage_b
...
def build_model(builder_cls):
    ...
    # Match FEM Stage B: gravity is OFF unless STAGEB_WITH_GRAVITY (Newton default
    # is -9.81, which previously left the slab sagging while FEM had no gravity).
    g = -params.GRAVITY if params.STAGEB_WITH_GRAVITY else 0.0
...
def main():
    parser = argparse.ArgumentParser(description="Newton XPBD rigid-sphere indentation (Stage B)")
    ...
    print(f"[newton-B] device = {device}")
    ...
    with wp.ScopedDevice(device):
        ...
        centre_line = top[np.abs(rest[top, 1] - cy) < 0.6 * params.STAGEB_CELL]
        ...
        for k in range(1, n_steps + 1):
            ...
            print(f"[newton-B] delta={delta * 1000:6.2f} mm  pen={pen * 1000:.2f} mm  "
        ...
        print(f"[newton-B] wrote {params.NEWTON_STAGEB_NPZ}")
        print(f"[newton-B] solve wall time = {wall_time:.3f} s")
