"""Stage A -- Newton's superpower: DIFFERENTIABLE simulation for evaluation.
...
We use this two ways, both against the FEM reference (data/fem_result.npz):
  ...
  2. INVERSE FIT (gradient descent):
     ...
     theta* < 1 means stiffer. This turns "XPBD looks ~10% soft" into a number
     obtained by differentiating through the physics.
...
Run on Colab (CUDA):  python -m newton_run.diffsim_stage_a
...
class DiffFit:
    def __init__(self, args):
        ...
        # clamp the top face (same as the forward Stage A run)
...
def main():
    parser = argparse.ArgumentParser(description="Differentiable stiffness fit (Stage A)")
    ...
    if not os.path.exists(params.FEM_NPZ):
        raise FileNotFoundError(f"{params.FEM_NPZ} missing -- run fenics_run.run_stage_a first")
